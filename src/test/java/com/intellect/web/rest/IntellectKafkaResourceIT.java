...
import com.intellect.config.KafkaProperties;
...
class IntellectKafkaResourceIT {
    ...
    void setup() {
        ...
        IntellectKafkaResource kafkaResource = new IntellectKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/intellect-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/intellect-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
